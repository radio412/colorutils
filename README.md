...
You may find that using log values derived from your data may lead to better differentiation between colors. A log function is supplied in the utility. You supply values, the log base and if you'd like to compensate for negative values. The last parameter which compensates for negative values preserve the value domain by shifting all values above zero by the lowest number encountered in the data. So a data set of -3, 1, and 4 becomes 0, 4, 8 because -3 is the lowest value and its absolute value is applied to every value in the set. A log is then produced capable of mapping a color. Such a domain shift and log conversion should be documented and explained to the user. The usage us simple.
